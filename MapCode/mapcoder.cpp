...
int ccode_of_iso3(const char *in_iso)
{
  ...
  if ( strncmp(s,iso,3)!=0 ) // not equal to cache?
  {
    ...
    if ( iso[2]==0 || iso[2]==' ' ) // 2-letter iso code?
    {
      ...
      if ( strncmp(s,disambiguate_iso3,3)!=0 ) // still not equal to cache?
      {
...
long fast_decode( const char *code )
{
  ...
  while (*code!='.' && *code!=0 )
  {
    value = value*31 + decode_chars[(unsigned char)*code++];
  }
  return value;
}
...
void decode_triple( const char *result, long *difx, long *dify )
{
      ...
      int c1 = decode_chars[(unsigned char)*result++]; 
      if ( c1<24 )
      {
...
int decode_nameless( const char *orginput, long *x, long *y,         int input_ctry, int codex, long debug_oldx, long debug_oldy )
{
  ...
  
  {
    ...
    if ( codex!=21 && A<=31 )
    {
      int offset = decode_chars[(unsigned char)*result]; 
    ...
    else if ( codex!=21 && A<62 ) 
    {
      ...
      X = decode_chars[(unsigned char)*result]; 
			if ( X < (62-A) )
			{
...
{ // rewrite all-digit codes
  char *s=input;
  char *dotpos=NULL;
  for (;*s!=0 && s[2]!=0;s++) if (*s=='-') break; else if (*s=='.' && !dotpos) dotpos=s; else if ( decode_chars[(unsigned char)*s]<0 || decode_chars[(unsigned char)*s]>9 ) return 0;  // nondigit, so stop
  if (dotpos)
  {
    ...
    {      
      ...
      else if (decode_chars[(unsigned char)*e]<0) return -9; // invalid last character!
      else v+=decode_chars[(unsigned char)*e];
...
//////////// AT THIS POINT, dot=FIRST DOT, input=CLEAN INPUT (no vowels) ilen=INPUT LENGTH
		...
		// check if input has valid characters
    {
      const char *s=input; 
      for ( ; *s!=0; s++ )
        if ( decode_chars[(unsigned char)*s]<0 && *s!='.' ) 
    ...
    *result=0;
    ...
    {      
      ...
      if (*result) // GOT A RESULT?
      {
        ...
        addresult(resultbuffer,result,x,y,the_ctry);
...
unsigned char *asdms(unsigned char *r,double lat,double lon,unsigned char degsym) // worst case EXcluding zts is 22 bytes: -##d##'##",-###d##'##"
{
  ...
  strcat((char*)r,",");
  asdms1(r+strlen((const char*)r),lon,180*3600,degsym);
  return r;
}
...
int interpret_coord( const unsigned char *i, int islat, double *result )
{
  ...
  {
    // get value
    double v=atof((const char*)i); skipfp(i); skipwhite(i);
...
int coord2mc( char **v, double lat, double lon, int tc )
{
  ...
  if (ccode==0)
  {
    ...
    for(i=0;i<MAX_CCODE;i++) {
      master_encode(NULL,i,(long)lon,(long)lat,-1,0,0,1);
    }
  }
  else
  {
    master_encode(NULL,ccode,(long)lon,(long)lat,-1,0,0,1);
  }
  global_results=NULL;
